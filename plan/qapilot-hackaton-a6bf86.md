# QAPilot Hackathon Plan

Create a GitHub issue improvement tool that uses GitHub's free models to automatically enhance ticket quality when users manually trigger it after updating issue titles.

## Requirements Confirmed

1. **Trigger Mechanism**: Manual trigger via GitHub UI (slash command or comment)
2. **Model**: GitHub's free models (GitHub Models API) - lowest resource consumption
3. **Scope**: Personal repositories only
4. **Output**: Direct ticket update after user finishes title update
5. **Interface**: Entirely within GitHub UI, no external apps
...
- Create GitHub Models API client (instead of Ollama)
- Design prompt templates for issue analysis
- Set up GitHub Actions workflow

### Phase 2: Manual Trigger System
- Implement comment-based trigger (`/improve-issue` command)
